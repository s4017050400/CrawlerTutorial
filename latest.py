...
metadata = get_paged_meta(start_url, num_pages=100)
for meta in metadata:
    #pretty_print(meta['push'], meta['title'], meta['date'], meta['author'])
    s1=str(meta["title"])
    try:
        fout=open('%s.txt'%s1,'wt')
    except :
        od=OrderedDict([("%s"%i,"_") for i in '\/:*?"<>|'])
        s1=replace_all(str(meta["title"]),od)#.replace("%s"%rp,"_")
        fout=open('%s.txt'%s1,'wt')
    str1=str(pretty_print(meta['push'], meta['title'], meta['date'], meta['author']))
    fout.write(str1)
