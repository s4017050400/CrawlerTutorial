...
def sort_out(datestr):
    ...
    for i in resp.text.split('\n'):
        ...
        if len(block) == 17 and i[0] != '=':
            del block[16] #del '\r'
            fstock=(block[0]+block[1]).replace('"','') #.translate({ord(c): None for c in '"'}) #1101台泥
            ...
            for b1 in block:
                b1clr=b1.replace('"','') #.translate({ord(c): None for c in '"'})
                pattern = '%s%s'
...
def datebyyear(year):
    ...
    for i in range(0,10*year): #1+365天 // 先秀抓10天
